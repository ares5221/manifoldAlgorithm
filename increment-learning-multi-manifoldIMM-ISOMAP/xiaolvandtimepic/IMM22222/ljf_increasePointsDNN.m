...
%   %{
    ...
    Dset = min(D1,D1');  Ws = Dset;    Ws(find(Ws==Inf)) = 0;    Ws = sparse(Ws);    Ws = Ws';
    ...
    [newDG,newP] = computeOneDG(size(XX,2)+1:size(XX,2)+size(X1,2),Ws2,newDG,newP);
   %为了提高精度，暂时先不考虑，注释掉
 %   [newDG,newP] = updateNewOne(jList,Ws2,newDG,newP);
    ...
    if(~isempty(allEdges))
       currJ = (allEdges(1,:)-1)*(size(XX,2)+size(X1,2))+allEdges(2,:);
       currJ1 = (allEdges(2,:)-1)*(size(XX,2)+size(X1,2))+allEdges(1,:);
       currJ = gunion(currJ,currJ1,1:(size(XX,2)+size(X1,2))^2);
