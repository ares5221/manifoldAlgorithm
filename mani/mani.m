...
switch exampleValue
    case 1  % Swiss Roll
        ...
        set(handles.ParamEdit,'String','1.0');                          %Load Example按钮下参数的设置在选择不同的value时候对应不同的Z Scaling
    case 2  % Swiss Hole
        set(handles.text19,'String','Z Scaling =');                     %或者Lift Angle或者'# Clusters或者Sample Rate或者Occluder r
...
%       See ISPC and COMPUTER.
if ispc
    set(hObject,'BackgroundColor','white');                 % 控制Parameters框Target Dimension第1个编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
...
%       See ISPC and COMPUTER.
if ispc
    set(hObject,'BackgroundColor','white');          % 控制Parameters框Nearest Neighbors第2个编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
...
%       See ISPC and COMPUTER.
if ispc
    set(hObject,'BackgroundColor','white');              % 控制Parameters框Sigma第3个编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
...
%       See ISPC and COMPUTER.
if ispc
    set(hObject,'BackgroundColor','white');            % 控制Manifole框Color Vector第3个编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
...
%       See ISPC and COMPUTER.
if ispc
    set(hObject,'BackgroundColor','white');                  % 控制Manifole框#Point第4个编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
...
switch exampleValue
    ...
    case 2  % Swiss Hole
        ...
        kl = repmat(0,1,2*N);    %repmat是一个处理大矩阵且内容有重复时使用，其功能是以0矩阵的内容行数乘以1也就是不重复，列数重复2*N次堆叠在K1的矩阵中
        for ii = 1:2*N
            if ( (tt(ii) > 9)&(tt(ii) < 12))
...
warning off;
[D,N] = size(X);         %返回矩阵X的行数和列数
% STEP1: COMPUTE PAIRWISE DISTANCES & FIND NEIGHBORS 
X2 = sum(X.^2,1);     %对X.^2每列分别求和
distance = repmat(X2,N,1)+repmat(X2',1,N)-2*X'*X;
[sorted,index] = sort(distance);    %sorted是原来的矩阵x按照行，每行从小到大重新排列得到的新矩阵。index告诉你重排的详细信息，也就是做了什么样的变动。
neighborhood = index(2:(1+K),:);     %找出最近的K个点
...
W = zeros(K,N);    %W是一个KxN的o矩阵
for ii=1:N
   z = X(:,neighborhood(:,ii))-repmat(X(:,ii),1,K); % shift ith pt to origin
   C = z'*z;                                        % local covariance
   C = C + eye(K,K)*tol*trace(C);                   % regularlization (K>D)
   W(:,ii) = C\ones(K,1);                           % solve Cw=1    %权值的约束条件两个
   W(:,ii) = W(:,ii)/sum(W(:,ii));                  % enforce sum(w)=1
end;
% STEP 3: COMPUTE EMBEDDING FROM EIGENVECTS OF COST MATRIX M=(I-W)'(I-W)
M = sparse(1:N,1:N,ones(1,N),N,N,4*K*N);     %由向量1:N,1:N,ones(1,N) 生成一个N*N，且最多含有4*K*N个元素的稀疏矩阵，ones(1,N)，对应要形成矩阵对应位置的值。
for ii=1:N                                                          %sparse([1,2,3,4],[1,2,3,4],[0,0,1,1],5,5,6)    ans =  (3,3) 1 ;   (4,4) 1。i 和j 的位置为一一对应，即(1,1)(2,2)(3,3)(4,4),将s中的值赋给这四个坐标的位置。若i=[2,1,3,4],j=[3,2,4,1]，则形成的坐标为(2,3)(1,2)(3,4),(4,1)
   w = W(:,ii);
   jj = neighborhood(:,ii);
   M(ii,jj) = M(ii,jj) - w';     %行向量
   M(jj,ii) = M(jj,ii) - w;      %列向量
   M(jj,jj) = M(jj,jj) + w*w';   %对角向量
end;
...
%       See ISPC and COMPUTER.
if ispc
    set(hObject,'BackgroundColor','white');                %控制Manifold框最后一个参数Height编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
...
if ispc
    set(hObject,'BackgroundColor','white');                %控制Parameters框最后一个参数Alpha编辑框颜色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
end
